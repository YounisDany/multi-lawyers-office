...
// جلب القضايا الموكلة للمحامي
$stmt = $pdo->prepare("SELECT c.*, u.name as client_name FROM cases c JOIN users u ON c.client_id = u.id WHERE c.lawyer_id = ? ORDER BY c.created_at DESC");
$stmt->execute([$user_id]);
$cases = $stmt->fetchAll();

// جلب الاستشارات الموجهة للمحامي
$stmt = $pdo->prepare("SELECT co.*, u.name as client_name FROM consultations co JOIN users u ON co.client_id = u.id WHERE co.lawyer_id = ? ORDER BY co.created_at DESC");
$stmt->execute([$user_id]);
...
<head>
    ...
    <title>لوحة تحكم المحامي - <?php echo SITENAME; ?></title>
    ...
    <ion-app>
        ...
        <ion-content class="ion-padding">
            <div class="content-section">
                <ion-card>
                    <ion-card-header>
                        <ion-card-title>قضاياي الموكلة</ion-card-title>
                    </ion-card-header>
                    <ion-card-content>
                        <?php if (!empty($cases)): ?>
                            <ion-list>
                                <?php foreach ($cases as $case): ?>
                                    <ion-item href="lawyer_case_details.php?id=<?php echo $case->id; ?>">
                        ...
                        <?php else: ?>
                            <div class="empty-state">
                                ...
                                <h3>لا توجد قضايا موكلة حالياً.</h3>
                            </div>
                        <?php endif; ?>
                    </ion-card-content>
                </ion-card>
                ...
                <ion-card class="ion-margin-top">
                    <ion-card-header>
                        <ion-card-title>استشارات العملاء</ion-card-title>
                    </ion-card-header>
                    <ion-card-content>
                        <?php if (!empty($consultations)): ?>
                            <ion-list>
                                <?php foreach ($consultations as $consultation): ?>
                                    <ion-item href="lawyer_consultation_details.php?id=<?php echo $consultation->id; ?>">
